...
http://www.codeskulptor.org/#user46_e2s0NrefWTl5Zc6.py


"Rice Rocks" -  a version of the game based on Asteroids, 
...
missile_info = ImageInfo([5,5], [10, 10], 3, 65)
...
def group_collide(group, other_object):
    ...
    for sprite in set(group):
        if sprite.collide(other_object):
            collision = True
            group.remove(sprite)
            explosion_group.add(Sprite(sprite.pos, sprite.vel,
                                       sprite.angle, 0, explosion_image_extra, 
                                       explosion_info_extra, explosion_sound))
    return collision    
...
def process_sprite(group, canvas):
    item_remove = set()
    for item in group:
        item.update()
        item.draw(canvas)
    ...
    group.difference_update(item_remove)
...
def draw(canvas):
    global time, started, score, lives, rock_group, highscore
    ...
    my_ship.draw(canvas)
    my_ship.update()
    process_sprite(rock_group, canvas)
    process_sprite(missile_group, canvas)
    process_sprite(explosion_group, canvas)
    ...
    if not started:
        ...
        canvas.draw_text(("High Score: " + str(highscore)), [(WIDTH // 2) - 80, (HEIGHT // 2) + 100], 20, 'White')



# timer handler that spawns a rock    
def rock_spawner():
    global rock_group, difficulty
